...
public class TransactionCreatorTest {
	...
	private void expectSameTransAction(User owner, User receiver) {
		assertThat(owner.getDepotItems().get(0).getTransaction(),
				is(owner.getDepotItems().get(0).getTransaction()));
	}

	private void expectBalance(User user, long amount) {assertThat(user.getBalance(), is(amount));}

	private void expectDepotItem(User user, long amount, User other, User.DepotItemType type) {List<User.DepotItem> depotItems = user.getDepotItems();
